<template>
  <!-- 新增编辑字典 -->
  <my-dialog :title="isAdd?'新增字典':'修改字典'" v-model="show">
...
<script>
import {sysDictTypeAdd, sysDictTypeUpdate} from '@/api/System/DictionType'
export default {
  data() {
    return {
      isAdd: true,
    ...
    submit(){
      ...
      vm.$refs.form.validate((valid) => {
        if (valid) {
          ...
          let handler = vm.isAdd ? sysDictTypeAdd : sysDictTypeUpdate;
          vm.loading = true;
          handler(params)
            .then(() => {
